...
async function loadGoodsDetailPage() {
  ...
  if (response.ok) {
    ...
    document.getElementById(
      "review_total"
    ).innerText = `리뷰 (${data["review_set"].length})`;
...
async function checkReview(goodsId) {
  const response = await fetch(
    `${backend_base_url}/store/check-review/${goodsId}`,
    {
      headers: {
        "content-type": "application/json",
        Authorization: `Bearer ${access_token}`,
...
async function reviewDetail(data, goodsId) {
  let check_review = checkReview(goodsId)
    .then((result) => {
      if (result === true) {
...
async function deleteReview(review_id) {
  const response = await fetch(
    `${backend_base_url}/store/review/${review_id}`,
    {
      headers: {
        "content-type": "application/json",
        Authorization: `Bearer ${access_token}`,
