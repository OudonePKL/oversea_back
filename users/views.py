...
class CheckEmailView(APIView):
    def post(self, request):
        ...
        return Response(
            {"message": "Email verification completed.", "is_checked": True},
            status=status.HTTP_200_OK,
        )
...
class SignupView(APIView):
    ...
    @swagger_auto_schema(
        tags=["join the membership"], request_body=PostUserSerializer, responses={200: "Success"}
    )
    def post(self, request):
            ...
            if serializer.is_valid():
                ...
                if category == "2":
                    ...
                    if not name or not address:
                        return Response(
                            {"message": "Please enter all required information."}, status.HTTP_400_BAD_REQUEST
                        )
                ...
                return Response(
                    {"message": "Your registration has been completed."}, status=status.HTTP_201_CREATED
                )
            else:
                return Response(
...
class LoginView(TokenObtainPairView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not is_user:
            return Response(data={'message': 'The username or password do not match.'}, status=400)
...
class UserView(APIView):
    ...
    def patch(self, request):
        ...
        try:
            ...
            # New Password & Confirm Password Confirm Match
...
class SocialUrlView(APIView):
    @swagger_auto_schema(
        tags=["social login"], request_body=PostUserSerializer, responses={200: "Success"}
    )
    def post(self, request):
...
class GoogleLoginView(APIView):
    ...
    def post(self, request):
        ...
        try:
            idinfo = google_id_token.verify_oauth2_token(id_token_str, Request(), client_id)
    ...
    def post(self, request):
        with transaction.atomic():
            ...
            if user.is_staff and not user.is_superuser:
                return Response(
                    {"message": "You are already registered as a seller."}, status.HTTP_400_BAD_REQUEST
                )
            ...
            if serializer.is_valid():
                ...
                return Response(data={"user_id": user.id, "store_id": serializer.data.get('id'),
                                      "user_name": user.nickname,
...
class CheckToken(APIView):
    ...
    def post(self, request):
        try:
            ...
            return Response(
                {"result": "fail"}, status=status.HTTP_200_OK
            )
